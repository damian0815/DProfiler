...
public:
	...
	~DTime() { };
	DTime( const DTime& other ) { Copy(other); }
	...
	DTime operator-( const DTime& other ) const
	{
        // copy self
        DTime t = *this;
        // invoke operator -=
	...
	// assignment
	DTime& operator= (const DTime& other ) { Copy(other); return *this; }

	// equality
	bool operator== (const DTime& other ) const {
	    #ifdef OSX
...
//inline bool operator< (const DTime& a, const DTime& b) { return a.operator<(b); }
